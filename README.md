...
* $(1)$ for $1$ dimensions,
* $(2, 1)$ for $2$ dimensions,
* $(4, 2, 1)$ for $3$ dimensions and
* maybe $(8, 4, 2, 1)$ for $4$ dimensions.
...
Move from corrector-only Euler method
to a more general predictor--corrector method.
...
Split integrators into predictor--corrector pairs.
...
Air has a dynamic viscosity of
$\\eta = \\SI{18.45}{\\micro \\Pascal \\second}$,
so the drag force felt by a $r = \\SI 1{\\centi \\meter}$ grain
moving at $v = \\SI 1{\\meter \\per \\second}$ is
$F = -b v = -3 \\twopi \\eta r v = -\\SI{3.48}{\\micro \\newton}$.
For water $\\eta = \\SI{890}{\\micro \\Pascal \\second}$,
so $F = -\\SI{168}{\\micro \\newton}$.

Try the gear integrator.
